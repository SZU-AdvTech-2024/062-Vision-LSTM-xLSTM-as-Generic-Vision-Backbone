...
class VisionLSTM2(nn.Module):
    ...
    def forward(self, x):
        ...
        # 重新调整维度以适应卷积层
        x = self.legacy_norm(x)  # 如果需要
        b, l, c = x.shape
        x = x.reshape(b, 16, 16, c)
        x = x.permute(0, 3, 1, 2)  # [batch, dim, 16, 16]

        # 应用头部卷积层，将通道数转换为1
        x = self.head(x)  # [batch, 1, 16, 16]

        # 上采样到目标尺寸 [256, 256]
        x = F.interpolate(x, size=(256, 256), mode='bilinear', align_corners=False)  # [batch,1,256,256]
...
if __name__ == '__main__':
    ...
    model = VisionLSTM2(dim=256,input_shape=(1,256,256),patch_size=16,depth=16,output_shape=(1,))
    model = model.to(device)
    # model = torch.nn.DataParallel(model)
    input_tensor = torch.randn(10, 1, 256, 256).to(dtype=torch.float32, device=device)
